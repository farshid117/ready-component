export const FOREGROUND =  "#f8f8f8";
export const BACKGROUND =  "#0a1045";
export const MYBLUE =      "#0d6efd";
export const blue =        "#007bff";
export const MYBLUE2 =     "#3498db";
export const LOGOCOLOR1 =  "#00c2d1";
export const CYAN =        "#8be9fd";
export const CURRENTLINE = "#44475a";
export const COMMENT =     "#6272a4";

export const YELLOW =      "#f9e900";
export const YELLOW4 =     "#f4b932";
export const YELLOW3 =     "#ffc44d";
export const YELLOW2 =     "#f1fa8c";

export const RED =         "#ff5555";
export const GHAHVEI =     "#f6af65";

export const PURPLE2 =     "#8e44ad";
export const PURPLE =      "#bd93f9";

export const JEEGARI2 =    "#EE2A7B";
export const PINK2 =       "#ff1756";
export const JEEGARI1 =    "#DA1C5C";
export const SOORATI =     "#ed33b9";
export const PINK =        "#ff79c6";

export const GREEN2 =      "#1cb65d";
export const GREEN =       "#50fa7b";

export const LOGOCOLOR2 =  "#02BDB2";


export const purple =      "#6610f2";
export const purple2 =     "#6f42c1";
export const pink =        "#e83e8c";
export const red =         "#dc3545";
export const orange =      "#fd7e14";
export const yellow =      "#ffc107";
export const green =       "#28a745";
export const teal =        "#20c997";
export const cyan =        "#17a2b8";
export const white =       "#ffffff";
export const gray =        "#6c757d";
export const grayDark =    "#343a40";
export const primary =     "#007bff";
export const secondary =   "#6c757d";
export const success =     "#28a745";
export const info =        "#17a2b8";
export const warning =     "#ffc107";
export const danger =      "#dc3545";
export const light =       "#f8f9fa";
export const dark =        "#343a40";
